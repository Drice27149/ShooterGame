- [10.31~11.6 UMG](#head1)
- [11.7~11.20 骨骼动画](#head2)
- [11.21~11.27 基本物理](#head3)
- [11.28~12.11 网络同步](#head4)
- [12.12~12.18 AI](#head5)
## <span id="head1">10.31~11.6 UMG</span>
...
## <span id="head2">11.7~11.20 骨骼动画</span>
...
## <span id="head3">11.21~11.27 基本物理</span>
...
## <span id="head4">11.28~12.11 网络同步</span>
...
## <span id="head5">12.12~12.18 AI</span>

* **演示视频**: [网盘链接](https://pan.baidu.com/s/1o57ZEvzofyP9jVA23i5Atg) 提取码: ka89 **打包文件**: [网盘链接](https://pan.baidu.com/s/1ZgysM-FcD1D1RAhIPZGdLA) 提取码: mi9v 

* **本次内容**: AI

  * 实现寻敌, 移动, 开火, 躲避四个基础结点

    * 寻敌使用了课堂上老师演示的方法, 每次随机移动到一个新的位置, 同时把```AI Perception```的```AI Sight sense```打开, 就可以在AI的视野范围内自动寻敌了。

    * 在确定了目标位置后, 移动使用行为树中的```move to```结点即可。

    * 由于我的AI角色是继承自我的玩家角色的, 所以开火只要调用之前写好的开火方法即可。单单这样是打不到人的, 因为没有瞄准。还需要在```AIController```中调用```Set focus```方法, 把瞄准的对象设置为玩家的角色。

    * 躲避需要用到EQS, 主要用于寻找合适的躲避位置。经过一些实验之后, 我的EQS设置是这样的:

      考虑以AI为中心的矩形区域内的所有点, 首先使用```Overlap```过滤出掩体(墙)附近的点, 再使用```trace```把玩家视野内的点排除(确保可以躲避玩家的开火), 再排除和当前距离过小(避免AI原地不动)和过大的点, 最后取出剩下的点中离当前位置最近的点作为躲避位置。

  * 实现多种性格的AI, 我实现了突击兵和机枪兵两种。

    *  突击兵使用抛体枪, 向玩家发射会爆炸的抛体, 并且突击兵在开火过程中会不断向玩家逼近, 即使被玩家击中也不会后退。
    *  机枪兵使用机关枪, 开火过程中原地战力不同, 在收到玩家的攻击后会第一时间躲避, 躲避的实现就是使用上面提到的EQS生成一个躲避位置, 然后```move to```到那个位置。
